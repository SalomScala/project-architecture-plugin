package com.projectarchitecturedescription.plugin.util;
...
public class Task {
	...
	private static void calculateElementsFromProjectDependencies(final Project p, final Set<ProjectElement> dependenciesSoFar) {
		p.getConfigurations().all((final Configuration c) -> {
			final DomainObjectSet<org.gradle.api.artifacts.ProjectDependency> projectDependencies =
					c.getDependencies().withType(org.gradle.api.artifacts.ProjectDependency.class);
	...
	public Dependencies calculateTransitiveDependencies() {
		...
		calculateTransitiveDependencies(new ProjectElement(this.task.getProject()), new LinkedHashSet<>(), dependencies);
	...
	private static void calculateTransitiveDependencies(final ProjectElement actual,
			final Set<ProjectElement> fromElements,
		...
		actual.getProject().getConfigurations().all((final Configuration c) -> {
			...
			projectDependencies.forEach((final org.gradle.api.artifacts.ProjectDependency p2) -> {
				...
				calculateTransitiveDependencies(new ProjectElement(p2.getDependencyProject()), newFromElements, dependencies);
			});
		});
	}
}
